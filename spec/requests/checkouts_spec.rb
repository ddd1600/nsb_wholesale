  ...
  context "#update" do
    ...
    context "Spree::Core::GatewayError" do
      ...
      before do
        ...
        # Solidus 4.7 renamed OrderUpdater#update_payment_state to
        # #recalculate_payment_state. The starter frontend shipped this spec
        # stubbing the old name, so the verifying double raised on the real
        # call instead of exercising the GatewayError path.
        allow(updater_instance).to receive(:recalculate_shipment_state)
        allow(updater_instance).to receive(:recalculate)
